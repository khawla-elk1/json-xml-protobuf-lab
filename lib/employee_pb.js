...
$root.Employees = (function() {
    ...
    Employees.encode = function encode(message, writer) {
        if (!writer)
            writer = $Writer.create();
        if (message.employee != null && message.employee.length)
            for (var i = 0; i < message.employee.length; ++i)
                $root.Employee.encode(message.employee[i], writer.uint32(/* id 1, wireType 2 =*/10).fork()).ldelim();
        return writer;
    };
    ...
    Employees.verify = function verify(message) {
        ...
        if (message.employee != null && message.hasOwnProperty("employee")) {
            if (!Array.isArray(message.employee))
                return "employee: array expected";
            for (var i = 0; i < message.employee.length; ++i) {
    ...
    Employees.fromObject = function fromObject(object) {
        ...
        if (object.employee) {
            if (!Array.isArray(object.employee))
                throw TypeError(".Employees.employee: array expected");
            message.employee = [];
            for (var i = 0; i < object.employee.length; ++i) {
                if (typeof object.employee[i] !== "object")
                    throw TypeError(".Employees.employee: object expected");
    ...
    Employees.toObject = function toObject(message, options) {
        ...
        if (message.employee && message.employee.length) {
            object.employee = [];
            for (var j = 0; j < message.employee.length; ++j)
                object.employee[j] = $root.Employee.toObject(message.employee[j], options);
        }
        return object;
    };
